# [Yunqiu Xu] [20200319] an sdvrp environment
# Currently this version is same with CVRP!
...
class SDVRPProblem(object):
    def __init__(self, graph_size=20, seed=1234):
        print("""
            The Split Delivery VRP (SDVRP) is a generalization of CVRP in which 
            every node can be visited multiple times, and only a subset of the 
            demand has to be delivered at each visit. I.E. splitted customer demands 
            over multiple routes.
            """)
        print("Warning! In current version SDVRP is same with CVRP!!!")
    ...
    def generate_test_dataset(self, dataset_size=1000, foldername="test_dataset/"):
        ...
        """
        filename = "{}sdvrp{}_{}_seed{}.pkl".format(foldername, self.graph_size, dataset_size, self.seed)
    ...
    def compute_cost(self, inputs, selected_idxs):
        """
        :param inputs: a batch of vrp data {depot:(batch_size, 2), 
    ...
    def plot(self, inputs, selected_idxs, 
        ...
        capacity = self.capacity_dict[self.graph_size] # e.g. for vrp100, the demand_scale is 50
        # setup figure
        ...
        plt.title("SDVRP{}, {} routes, total distance {:.2f}".format(self.graph_size, len(routes), total_dist))
